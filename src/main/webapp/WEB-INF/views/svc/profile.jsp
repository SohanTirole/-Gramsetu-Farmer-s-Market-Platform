...
<c:set var="pageTitle" value="SVC Profile"/>
<div style="max-width:640px;margin:0 auto">
  <div class="page-header"><div><h1>SVC Profile</h1></div></div>
  <div class="card mb-3" style="display:flex;align-items:center;gap:20px;padding:28px">
    <div style="width:68px;height:68px;border-radius:50%;background:var(--gold);display:flex;align-items:center;justify-content:center;font-family:var(--font-head);font-size:26px;font-weight:700;color:var(--white)">${user.initials}</div>
    <div><h2 style="font-family:var(--font-head);font-size:1.2rem">${user.name}</h2>
      <p class="text-muted">${user.email}</p><span class="badge badge-warning mt-1">SVC ENTREPRENEUR</span></div>
  </div>
  <div class="card">
    <div class="card-header"><span class="card-title">Update Profile</span></div>
    <form action="${pageContext.request.contextPath}/svc/profile/update" method="post">
