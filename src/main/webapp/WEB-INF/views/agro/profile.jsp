...
<c:set var="pageTitle" value="Agronomist Profile"/>
<div style="max-width:640px;margin:0 auto">
  ...
  <div class="card mb-3" style="display:flex;align-items:center;gap:20px;padding:28px">
    <div style="width:68px;height:68px;border-radius:50%;background:var(--forest-mid);display:flex;align-items:center;justify-content:center;font-family:var(--font-head);font-size:26px;font-weight:700;color:var(--white)">${user.initials}</div>
    <div><h2 style="font-family:var(--font-head);font-size:1.2rem">${user.name}</h2>
      <p class="text-muted">${user.email}</p><span class="badge badge-success mt-1">AGRONOMIST</span></div>
  </div>
  <div class="card">
    <div class="card-header"><span class="card-title">Update Profile</span></div>
    <form action="${pageContext.request.contextPath}/agro/profile/update" method="post">
      ...
      <div class="form-row-three">
        <div class="form-group"><label class="form-label">Village/City</label><input type="text" name="village" class="form-control" value="${user.village}"></div>
